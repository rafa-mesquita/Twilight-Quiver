...
func _apply_tick() -> void:
	...
	for enemy in _enemies_inside.duplicate():
		...
		if enemy.has_method("take_damage"):
			...
			var dmg_ft: float = amount
			# Crit roll por tick (DoT, mín +1 bônus).
			if p_for_crit != null and p_for_crit.has_method("roll_crit_dot"):
				var crit_ft: Dictionary = p_for_crit.roll_crit_dot(dmg_ft)
				dmg_ft = float(crit_ft.get("dmg", dmg_ft))
				if bool(crit_ft.get("crit", false)):
					CritFeedback.mark_next_hit_crit(enemy)
			enemy.take_damage(dmg_ft)
			_notify_player_dmg_kill(dmg_ft, "fire_arrow", was_alive_ft, enemy)
